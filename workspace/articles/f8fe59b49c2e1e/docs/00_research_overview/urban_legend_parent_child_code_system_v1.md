...
**設計メモ:** 最初に確認できる流通回路。Parent/Child taxonomyはD03と共用する。
...
**設計メモ:** 現在または主要流通期の媒体。主1、副0–2。D02との差が媒体移行を示す。
...
| Child ID | Child | 定義 |
|---|---|---|
| `D16.IMS.IMMEDIATE` | 即時 | 接触直後に作用・結果が起きる。 |
| `D16.IMS.SINGLE_OBSERVATION` | 単発観測 | 一度の出現・経験で完結する。 |
...
| `D16.DLY.LATENT` | 潜伏 | 一定期間潜伏後に発現する。 |

## `D16.PRG` — 進行・長期

| Child ID | Child | 定義 |
|---|---|---|
| `D16.PRG.STAGED_PROGRESSION` | 段階進行 | 段階的に悪化・進展する。 |
...
| Child ID | Child | 定義 |
|---|---|---|
| `D16.REC.RECURRENT` | 再発 | 不定期に繰り返す。 |
| `D16.REC.PERIODIC` | 周期 | 毎年、毎夜等の周期で起きる。 |
| `D16.REC.TRIGGERED_RECURRENCE` | 条件再発 | 同じ条件が成立するたび再発する。 |

## `D16.CON` — 持続・追跡

| Child ID | Child | 定義 |
|---|---|---|
| `D16.CON.CONTINUOUS` | 連続持続 | 切れ目なく継続する。 |
