...
repository path、Review Seq、開始artifact、Notion page ID、pre/post SHA等を通常の公開引数として要求しない。

## 3.2. 想定外部入口

実装する場合の公開CLIは、次の1入口とする。

```bash
python -m src.orchestration.run_entry_pipeline <Entry_ID>
```

本書作成時点でこのPython入口が未実装の場合、本節は**実装契約**として扱う。
...
- Workflow 90内部の `PASS / UPDATED / BLOCKED / ERROR` をそのまま外部結果として露出する必要はない。
- Workflow 00は正常完了時に `PASS` のみ返す。
- 詳細な中間状態は必要時のみログ・reason codeへ残す。
...
> **CORRECTION_REQUIRED状態で本番Entry pipelineが継続実行され、Workflow 10の修正Phaseへ入ること**

をCoder開始意思と定義する。

したがって、公開CLIへ追加引数を増やさず、

```text
run_entry_pipeline <Entry_ID>
```

という本番E2E実行そのものをcorrection startの明示イベントとして利用する。

実装時には、このイベントをWorkflow 90 / status managementへ渡す内部入口を用意する。

- 公開引数は増やさない。
- Entry_IDだけから「勝手に修正開始した」と推測するのではない。
- **Workflow 00がCORRECTION_REQUIREDを確認し、実際にcorrection phaseへ遷移した事実**を開始イベントにする。

この内部イベントが未実装の間は、`要修正 -> 再作業中` の自動遷移を完了扱いにしない。
...
```text
src/orchestration/run_entry_pipeline.py
```
...
Workflow 00のPython実装は、既存のdeterministic module内部実装をコピーしない。
...
↓
run / hand off Workflow 10
↓
validate
↓
run / hand off Workflow 20
↓
inspect verdict class
↓
iterate or finalize
```
