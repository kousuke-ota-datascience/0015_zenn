...
同一1986年資料群で赤／青／白、赤／青／黄、赤／紫が併存し、色・帰結が置換され、第三色・安全回答等が追加されていることは確認できる。ただし、それらが具体的にどの口承伝達過程で変形・増補されたかは直接観察していないため、生成・変容パターンとしてはIとする。

**この伝承における現れ方**

学校トイレで問いを受け、色を選び、回答内容に対応した結果が生じるという共通展開を持ちながら、選択肢と帰結が学校・語りごとに差し替えられる。
...
日文研DB要約と採録書誌から、受容者へ「共同体既知事実」「条件付き信念」「真偽未確定」のどの認識論的スタンスで提示されたかを一意に固定できない。学校伝承であることだけから真実性提示を推測しない。
...
この伝承がなければ、通常の日常施設である学校トイレの一部が、なぜ避けるべき危険な場所なのかを説明する共有モデルがない。また、そこで応答を迫る正体不明の存在も未説明のまま残る。単に舞台がトイレだから場所コードを選ぶのではなく、通常空間を危険空間へ再分類すること自体をPrimaryとする。
...
1986年の複数採録では、遭遇者は学校トイレ／特定個室にいることで正体不明の問いかけを受け、その時点ですでに怪異との因果関係へ入っている。色への回答は、因果系へ入った後に危害／安全の帰結を分岐させる条件である。したがって中心質問「何を契機に因果系へ入るか」へ最も直接答える `LOCATION_STATE` をPrimaryとし、`RESPOND_CHOOSE` をSecondaryへ置く。
...
1972年兵庫と1986年東京の異なる地域で学校児童・生徒圏の採録が確認できる。受容集団としては学校・若者文化が最も直接的であり、地理的には少なくとも単一学校・単一市町村に限定されないためREGIONALをSecondaryとする。全国的大衆までの拡張は行わない。
