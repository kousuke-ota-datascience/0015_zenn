...
1972年刊行の兵庫県尼崎市採録により、学校便所で色を問う早期型が少なくとも1972年までに流通していたことは確認できる。しかし、これは最古確認時点であり、生成時期そのものを1970年代へ限定する独立Evidenceではない。1956年の類似句も問い手・答え手・因果機能が異なり、直接祖型として生成時期を遡らせる根拠にはしない。

**この伝承における現れ方**

1972年までに早期型が存在したことは確認できるが、いつ成立したかは現Evidenceから一意に固定しない。
...
1972年赤／白型と1986年東京都の複数色選択型、さらに1986年同時点で併存する赤／青／白、赤／青／黄、赤／紫の差異はEvidenceとして確認できる。しかし、地域も時点も異なる資料間の差から、どの型がどの型から派生したか、口承過程で何が置換・増補されたかという変容方向・機構までは固定できない。同時点のVariation並存も、それ自体では `ORAL_VARIATION` や `ACCRETION` の生成過程を直接証明しない。

**この伝承における現れ方**

色選択型に複数のVariationが存在することは確認できるが、それらが時間とともにどのような機構で形成されたかは未確定として保持する。
