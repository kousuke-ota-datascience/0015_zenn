...
Version Scopeでは「口が裂けた女性が子どもを襲う」という主体構造までは固定できるが、その主体が通常の人間、妖怪、死者霊、その他の怪異存在のいずれであるかは一意に固定できない。後代の妖怪化を採用しないことを、通常人間であるという積極Evidenceへ変換しない。

**この伝承における現れ方**

危険主体は「口裂け女」という名前付き女性像として共有される一方、それを世界内でどの存在カテゴリーへ置くかは初期Scopeでは未決定のまま残る。
...
- `L3=0`: Version Scopeを1979年初頭〜同年春に限定する。1979年6月15日『秋田魁新報』の登校拒否はScope後続のX Evidenceであり、このScoped VersionのL3へは流用しない。春までに噂流通による独立した社会行動・制度変化を示すX Evidenceは現00で固定していない。

L1には直接Evidenceがある一方、L2=0とL3=0は不在判定を含むため、binary vector全体のStatusは保守的に `I` とする。

**この伝承における現れ方**

伝承内部では子どもへの襲撃が語られる。一方、伝承を聞くだけで同じ怪異作用を受ける構造や、春までのScope内で噂流通が独立した社会行動・制度変化を生んだことは確認しない。6月の登校拒否は後続の流通史・X Evidenceとして00側に保持する。
