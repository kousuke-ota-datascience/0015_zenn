<!-- GENERATED VIEW. DO NOT EDIT AS CANONICAL DATA. -->
<!-- Source JSON: docs/10_each_lore/0362_toyokawa_shinkin_bank_run/0362_20_analysis.json -->

# 0362 Analysis

- Macro category: 社会流言・パニック
- Entry type: 金融流言事件

## Version Scope

1973年の豊川信用金庫をめぐる流言が預金引き出しの集中・取付け騒ぎへ発展し、警察・関係機関の発端解明と真相の明示で沈静化した事件核。警察白書が明示する石油危機下の社会・人心の不安定な背景を含むが、具体的な流言文言は固定しない。

- Included variants: VAR-001
- Excluded variants: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004, CNT-005

## Dimensions

### D01 — D

- Primary: D01.G3
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: EVD-001

**Rationale:** 事件は1973年に発生した。

**Manifestation:** 1970年代。

### D02 — U

- Primary: -
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 最初の流言がどの媒体で人から人へ伝わったかを、今回の直接確認資料だけでは安全に固定できない。

**Manifestation:** 最古流通媒体不明。

### D03 — U

- Primary: -
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 個々の流通媒体ポートフォリオを直接確認する資料が不足する。

**Manifestation:** 流通媒体ポートフォリオ不明。

### D04 — U

- Primary: -
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** current Evidenceは流言の伝播拡大を示すが、パロディ・亜種・派生ルール等の派生版生成までは示さない。

**Manifestation:** 生成・変容パターンは未確定。

### D05 — I

- Primary: D05.PRP.FACT_CLAIM (parent: D05.PRP)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 具体的文言は不明だが、金融不安を伴う流言として受容され預金引き出しへ波及したことから、何らかの事実命題として提示されたと推定する。

**Manifestation:** 具体文言未確定の金融不安に関する事実主張。

### D06 — I

- Primary: D06.T4
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 流言が豊川信用金庫をめぐる金融不安として受容されたことから、制度的事実に関する内容として提示されたと推定する。

**Manifestation:** 金融機関に関する事実らしい内容として受容。

### D07 — I

- Primary: D07.INO.SYSTEM_CAPACITY_FAILURE (parent: D07.INO)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 取付けに至る金融不安の意味形成対象として、金融機関が預金を安全に保持できるかという制度能力への不安が中心だったと推定する。具体的な危険・損失文言は固定しない。

**Manifestation:** 金融機関の制度能力・安全性への不安。

### D08 — I

- Primary: D08.CLM.UNSUPPORTED_ASSERTION (parent: D08.CLM)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** Scoped Contentが事件の起点を流言として記述するため、根拠の提示を伴わない主張系の契機と推定する。ただし『根拠未提示』自体をEvidenceから直接持ち込んでDirectとは判定しない。

**Manifestation:** 流言として受容された主張。

### D09 — I

- Primary: D09.CAT.BOUNDARY_CLASSIFICATION (parent: D09.CAT)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 金融不安として受容され預金維持・引き出しの判断が変化したことから、豊川信用金庫を安全側に置けないものとして分類する認知操作を含むと推定する。具体的な危険文言は固定しない。

**Manifestation:** 金融機関を安全側に置けないものとして扱う分類。

### D10 — D

- Primary: D10.OBJ.INFORMATION_CONTENT (parent: D10.OBJ)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 直接の因果源は金融不安を含む情報内容としての流言。

**Manifestation:** 情報内容が行動を駆動する。

### D11 — D

- Primary: D11.INF.RECEIVE_MESSAGE (parent: D11.INF)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 流言を受け取ることが行動変化の入口。

**Manifestation:** メッセージ受信。

### D12 — D

- Primary: D12.AUD.READER_LISTENER (parent: D12.AUD)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** D13の情報誘発作用を直接受けるのは、流言を受け取って金融判断を変える預金者・受容者である。豊川信用金庫は多数の引き出しによる下流の影響対象として区別する。

**Manifestation:** 流言を受け取る預金者・受容者。

### D13 — D

- Primary: D13.COG.INFORMATION_INDUCED_ACTION (parent: D13.COG)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 流言情報が預金引き出しという行動を誘発する。

**Manifestation:** 情報誘発行動。

### D14 — D

- Primary: D14.NEG
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 金融機関と預金者双方に混乱を生じさせる。

**Manifestation:** 負。

### D15 — D

- Primary: D15.COL.MARKET_SOCIAL_BEHAVIOR (parent: D15.COL)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 個々の預金引き出しが集積し、取付け騒ぎという市場・集団行動上の変化に至ることがVersion Scopeの主帰結である。

**Manifestation:** 預金引き出しの集中と取付け騒ぎ。

### D16 — D

- Primary: D16.EVT.SEQUENTIAL_EPISODE (parent: D16.EVT)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 同一事件内で、流言の受容・拡大、預金引き出しの集中・取付け、発端解明・沈静化という複数段階が順に進む。流言の人から人への拡散自体をTRNの十分条件にはしない。

**Manifestation:** 一つの事件内で段階的に進むエピソード。

### D17 — D

- Primary: D17.INF.VERIFY_DEBUNK (parent: D17.INF)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: EVD-002

**Rationale:** 警察が発端を確認し真相を明示することが沈静化手段となった。

**Manifestation:** 検証・訂正。

### D18 — D

- Primary: D18.L3
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: EVD-001, EVD-002

**Rationale:** 流言が現実の預金引き出し・取付け騒ぎを生じさせたことが公的記録で確認される。

**Manifestation:** 社会現実層の実行動。

### D19 — U

- Primary: -
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003
- Evidence refs: -

**Rationale:** Scoped Contentは1973年に愛知県の豊川信用金庫をめぐる流言が広まり取付け騒ぎが発生したことを示すが、流言がどの社会範囲まで流通したかは特定できない。事件所在地を流通範囲へ置換せず、資料不足としてUとする。

**Manifestation:** 社会的・地理的な流通範囲は未確定。

### D20 — I

- Primary: D20.ORG.INSTITUTION_AUTHORITY (parent: D20.ORG)
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: -

**Rationale:** 金融機関の実際の経営状態や事件原因の確認には銀行・警察等の制度的情報が優位に位置する。

**Manifestation:** 制度機関が検証情報を保持。

### D21 — D

- Primary: D21.A4
- Secondary: -
- Content refs: CNT-001, CNT-002, CNT-003, CNT-004
- Evidence refs: EVD-001, EVD-002

**Rationale:** 実在する豊川信用金庫と1973年の取付け事件が伝承モデルそのものを構成する。

**Manifestation:** 史実・記録を因果統合。

## Analysis Notes

分析単位は流言イベント型を維持する。具体的な流言文言は固定せず、D12（流言受容者）→D13（情報誘発行動）→D15（市場・社会行動変化）の因果列を中心に置く。D16は流言の流通期間ではなく、同一事件内の段階進行として裁定する。D19は事件所在地と流通範囲を区別し、現行Contentでは社会的・地理的流通範囲を固定できないためUとする。石油危機下の社会・人心の不安定な背景はEVD-005/CNT-005で保存する。
