...
from .config_schema import DEFAULT_ALPHA_GRID, DEFAULT_DISCRETE_PC_INDEP_TESTS, DEFAULT_PC_INDEP_TESTS, FeatureConfig
from .graph_utils import graph_edge_records, weight_matrix_edge_records
from .schemas import DiscoveryResult


class CausalDiscovery:
    """Run causal discovery algorithms on a standardized feature frame.
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the discovery runner and validate algorithm settings.
    ...
    def prepare_pc_frame(self, frame: pd.DataFrame) -> pd.DataFrame:
        """Prepare PC input according to the selected independence test.
    ...
    def discretize_series(self, series: pd.Series) -> pd.Series:
        """Quantile-discretize a series for discrete PC tests.
    ...
    def run_ges(self, frame: pd.DataFrame):
        """Run the GES score-based search.
    ...
    def run_notears(self, frame: pd.DataFrame):
        """Run a linear NOTEARS implementation.
    ...
    def build_background_knowledge(self, node_names: list[str]):
        """Build causal-learn background knowledge from feature tiers.
