"""Low-level linear model estimation utilities."""
...
def normal_p_value(z_value: float) -> float:
    """Compute a two-sided normal-approximation p-value.
...
def confidence_interval(
    ...
    *,
    ...
    """Construct a normal-approximation confidence interval.
...
def none_to_nan(value: float | None) -> float:
    """Convert an optional numeric value to a float with ``nan`` for missing.
...
def numeric_matrix(frame: pd.DataFrame, columns: list[str] | tuple[str, ...]) -> np.ndarray:
    """Convert selected columns to a numeric matrix.
...
def fit_linear_regression(
    ...
    *,
    ...
    """Fits an OLS regression with optional heteroskedasticity-robust SE.
