"""Treatment-effect analysis mode."""
...
class TreatmentEffectModeStrategy(AnalysisModeStrategy):
    """Runs the treatment effect estimation mode."""

    mode = "treatment_effect"

    def run(self, context: RunContext) -> None:
        """Runs treatment effect estimation and writes outputs.
    ...
    def load_graph_edges_for_adjustment(self, context: RunContext) -> pd.DataFrame | None:
        """Load discovery edges when graph-parent adjustment is requested.
    ...
    def build_propensity_overlap(
        ...
        """Build propensity overlap output from the last propensity fit.
